		...
		catch ( SQLException e )
		{
			try 
			{
				if (DBConfig.getInstance().qualifyPolicy(
					...
					return;
				if ( this.autoCommit.equals(java.lang.Boolean.FALSE) && DBConfig.getInstance().qualifyPolicy( jdbcConn.getMetaData().getDriverName(), DBConfig.TRY_COMMIT_THEN_CLOSE ))
				{
					jdbcConn.commit();
					jdbcConn.close();
					return;
